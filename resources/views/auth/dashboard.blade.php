...
<div class="container mx-auto max-w-7xl py-8">
    <h1 class="text-3xl font-extrabold text-gray-900 mb-6">{{ __('auth.yourdashboard') }}</h1>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-xl shadow-lg p-6 transform transition-all duration-300 hover:scale-[1.01]">
            <h2 class="text-xl font-semibold text-gray-800 mb-4">{{ __('auth.Profile Information') }}</h2>
            <div class="flex items-center mb-4">
                ...
                @endphp
                <img src="{{ $profilePhotoUrl }}" alt="{{ __('auth.Profile Photo') }}" class="w-20 h-20 rounded-full object-cover mr-4 border-2 border-primary-500 shadow-md">
                <div>
            ...
            <p class="text-gray-700 mb-2">
                <strong>{{ __('auth.Address') }}:</strong>
                @if (Auth::user()->address)
                    {{ Auth::user()->address }}, {{ Auth::user()->city }} <br>
                    {{ Auth::user()->country }} ({{ __('auth.Phone') }}: {{ Auth::user()->phone_number ?? 'N/A' }})
                @else
                    <span class="text-gray-500 italic">{{ __('auth.Not set') }}</span>
                @endif
            </p>
            <div class="mt-4">
        ...
        <div class="bg-white rounded-xl shadow-lg p-6 transform transition-all duration-300 hover:scale-[1.01]">
            <h2 class="text-xl font-semibold text-gray-800 mb-4">{{ __('auth.Your Notifications') }}</h2>
                <ul class="space-y-3 text-gray-700">
                    @forelse(auth()->user()->unreadNotifications()->latest()->take(5)->get() as $n)
                        @php
                            $k = $n->data['key'] ?? null;       // 'welcome'
                            $p = $n->data['params'] ?? [];
                            $l = $n->data['locale'] ?? app()->getLocale();
                        @endphp
                        ...
                        <li class="flex items-start">
                            ...
                            <div @if($l === 'ar') dir="rtl" class="text-right" @endif>
                                <p>
                                    <strong>{{ __('auth.notifications.'.$k.'.subject', $p, $l) }}</strong><br>
                                    {{ __('auth.notifications.'.$k.'.line1', $p, $l) }}
                                </p>
                                @if(!empty($n->data['url']))
                                    <a href="{{ $n->data['url'] }}" class="text-primary-600 underline">
                                        {{ __('auth.notifications.'.$k.'.action', [], $l) }}
                                    </a>
                                @endif
                            </div>
                        </li>
                    @empty
                        <li class="text-gray-500">{{ __('auth.No tienes notificaciones por ahora.') }}</li>
                    @endforelse
                </ul>
            <p class="text-gray-500 text-sm mt-4">
                {{ __('auth.Stay up to date with the latest news and important messages') }}
            </p>
        </div>

        <!-- Acciones rápidas -->
        <div class="bg-white rounded-xl shadow-lg p-6 transform transition-all duration-300 hover:scale-[1.01]">
            <h2 class="text-xl font-semibold text-gray-800 mb-4">{{ __('auth.Quick Actions') }}</h2>
            <div class="grid grid-cols-2 gap-4">
                <a href="{{ route('profile.edit') }}" class="flex flex-col items-center justify-center p-3 bg-primary-100 text-primary-800 rounded-lg hover:bg-primary-200 transition-colors duration-200">
                    ...
                    <span class="text-xs font-medium">{{ __('auth.Edit Profile') }}</span>
                </a>
                <a href="{{ route('profile.edit') }}#update-password-section" class="flex flex-col items-center justify-center p-3 bg-primary-100 text-primary-800 rounded-lg hover:bg-primary-200 transition-colors duration-200">
                    ...
                    <span class="text-xs font-medium">{{ __('auth.Change Password') }}</span>
                </a>
        ...
        <div class="bg-white rounded-xl shadow-lg p-6 col-span-1 md:col-span-2 transform transition-all duration-300 hover:scale-[1.01]">
            <h2 class="text-xl font-semibold text-gray-800 mb-4">{{ __('auth.Recent Activity') }}</h2>
            <ul class="space-y-3">
                <li class="flex items-center text-gray-700">
                    ...
                    <p>{{ __('auth.Account registration completed') }} - {{ now()->format('d') }} de {{ now()->translatedFormat('F') }} de {{ now()->format('Y') }}</p>
                </li>
                <li class="flex items-center text-gray-700">
                    <span class="bg-primary-100 text-primary-700 rounded-full h-8 w-8 flex items-center justify-center mr-3">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                    ...
                    <p>{{ __('auth.Successful login') }} - {{ now()->format('d') }} de {{ now()->translatedFormat('F') }} de {{ now()->format('Y') }}</p>
                </li>
                <li class="flex items-center text-gray-700">
                    <span class="bg-primary-100 text-primary-700 rounded-full h-8 w-8 flex items-center justify-center mr-3">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                    ...
                    <p>{{ __('auth.Profile update') }} - {{ now()->format('d') }} de {{ now()->translatedFormat('F') }} de {{ now()->format('Y') }}</p>
                </li>
            </ul>
            <p class="text-gray-500 text-sm mt-4">
                {{ __('auth.Here you will see a summary of your recent activity on the platform') }}
            </p>
        </div>
